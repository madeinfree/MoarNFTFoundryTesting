...
contract MoarBase is ERC721, IERC2981, Ownable {
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    "https://metadata.thefwenclub.com/moar/",
                    tokenId.toString()
                )
            );
    }
    ...
    function toggleFlag(uint256 flag) public virtual onlyOwner {
        if (flag == uint256(keccak256("BURN"))) burningActive = !burningActive;
        else if (flag == uint256(keccak256("TRANSFER")))
            transferDeactive = !transferDeactive;
        else if (flag == uint256(keccak256("METADATA"))) metadataFrozen = true;
    }
    ...
    function burn(uint256 tokenId) external {
        if (!burningActive) {
            revert BurnningInactive();
        }
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) {
            revert NonOwnerOrApproved();
        }
        _burn(tokenId);
    }
     ...
     */
    function setRoyaltyInfo(address royaltyReceiver, uint256 royaltyPercentage)
        public
        onlyOwner
    {
        if (royaltyPercentage > 100) {
            revert RoyaltyPercentageExceed();
        }
    ...
    function _transfer(
        ...
        if (transferDeactive) {
            revert TransferDeactive();
        }
        if (transferLocks[tokenId] != LockStatus.Unlock) {
    ...
    function lockTransfer(uint256 tokenId, bool locked) external {
        ...
        if (transferLocks[tokenId] == LockStatus.LockByAdmin) {
            revert TransferLockedByAdmin();
        }

        transferLocks[tokenId] = locked
            ? LockStatus.LockByTokenOwner
            : LockStatus.Unlock;
     ...
     */
    function lockTransfers(uint256[] memory tokenIds, bool[] memory locks)
        external
        onlyAdmin
    {
        if (tokenIds.length != locks.length) {
            revert ArrayLengthMismatch();
        }

        for (uint256 i = 0; i < tokenIds.length; i++) {
            transferLocks[tokenIds[i]] = locks[i]
                ? LockStatus.LockByAdmin
                : LockStatus.Unlock;
            emit LockTransfer(
                ERC721.ownerOf(tokenIds[i]),
                tokenIds[i],
                locks[i]
            );
        }
