...
contract Moar is MoarBase, ReentrancyGuard {
    ...
    uint256 public constant DUTCH_AUCTION_DROP_PER_STEP =
        (DUTCH_AUCTION_START_PRICE - DUTCH_AUCTION_END_PRICE) /
            (DUTCH_AUCTION_PRICE_CURVE_LENGTH / DUTCH_AUCTION_DROP_INTERVAL);
    ...
    function toggleFlag(uint256 flag) public override onlyAdmin {
        if (flag == uint256(keccak256("SALE"))) saleOn = !saleOn;
        else super.toggleFlag(flag);
    }
     ...
     * - the caller must be `owner`.
     */
    function setDutchAuctionStartTime(uint256 startTime)
        external
        isSaleOn(false)
        onlyAdmin
    {
     ...
     * - the caller must be `_admin`.
     */
    function privateMint(address[] memory tos)
        external
        onlyAdmin
        isSaleOn(false)
    {
    ...
    function _publicMint(
        ...
        if (publicMints[msg.sender] + ticketNum > maxNum) {
            revert ExceedMaxTicket();
        }
    ...
    ) external payable isSaleOn(true) validTime(tierId) {
        bytes32 data = keccak256(abi.encode(address(this), msg.sender, tierId));

        if (
            !SignatureChecker.isValidSignatureNow(_authority, data, signature)
        ) {
            revert InvalidSignature();
        }

        _publicMint(
            tiers[tierId].maxTicketNum,
            ticketNum,
            tiers[tierId].ticketPrice,
            false
        );
    }
    ...
    function dutchAuctionMint(uint256 ticketNum, bytes memory signature)
        external
        payable
        isSaleOn(true)
        validateCaller
    {
        ...
        bytes32 data = keccak256(
            abi.encode(address(this), msg.sender, keccak256("DUTCH"))
        );
        if (
            !SignatureChecker.isValidSignatureNow(_authority, data, signature)
        ) {
        ...
        uint256 availableNum = Math.min(
            ticketNum,
            DHDAMaxSupply - DHDATotalSupply
        );
        if (availableNum <= 0) {
            revert ExceedAuctionSupply();
        }
        DHDATotalSupply += availableNum;

        _publicMint(
            DUTCH_AUCTION_MAX_TICKET,
            availableNum,
            dutchAuctionPrice(),
            false
        );
    ...
    function dutchAuctionPrice() public view returns (uint256) {
        ...
        if (timestamp < dutchAuctionStartTime) return DUTCH_AUCTION_START_PRICE;

        if (
            timestamp - dutchAuctionStartTime >=
            DUTCH_AUCTION_PRICE_CURVE_LENGTH
        ) return DUTCH_AUCTION_END_PRICE;

        uint256 steps = (timestamp - dutchAuctionStartTime) /
     ...
     * - caller must not be a contract.
     */
    function diamondHandMint(uint256 ticketNum, bytes memory signature)
        external
        payable
        isSaleOn(true)
        validateCaller
        validTime(DIAMOND_HAND_ID)
    {
        bytes32 data = keccak256(
            abi.encode(address(this), msg.sender, DIAMOND_HAND_ID)
        );
        if (
            !SignatureChecker.isValidSignatureNow(_authority, data, signature)
        ) {
        ...
        uint256 availableNum = Math.min(
            ticketNum,
            diamondHandMaxSupply - DHDATotalSupply
        );
        if (availableNum <= 0) {
            revert ExceedDiamondHandSupply();
        }
        DHDATotalSupply += availableNum;
        ...
        _publicMint(
            DUTCH_AUCTION_MAX_TICKET,
            availableNum,
            tiers[DIAMOND_HAND_ID].ticketPrice,
            true
        );
    }
